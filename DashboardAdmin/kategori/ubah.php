...
$kategoriLama = $_GET["kategori"] ?? '';
if ($kategoriLama === '') {
    header("Location: index.php");
    exit;
}
$data = queryReadData("SELECT * FROM kategori_buku WHERE kategori = '" . mysqli_real_escape_string($connection, $kategoriLama) . "'");
if (empty($data)) {
    header("Location: index.php");
    exit;
}
if (isset($_POST["simpan"])) {
    ...
    if (updateKategori($kategoriLama, $kategoriBaru) >= 0) {
        header("Location: index.php");
        exit;
    }
...
<body>
    ...
    <div class="container-plain">
        <h1 class="section-title">Edit Kategori Buku</h1>
        <div class="card-plain">
            <form action="" method="post">
                <div class="form-group">
                    <label>Nama Kategori</label>
                    <input type="text" name="kategori" value="<?= htmlspecialchars($kategoriLama); ?>" required>
                </div>
